...
- `render()` 根据 `provider` 参数选择标签（5h/W/M vs 4h/W）和颜色语义
- 火山方舟 `Percent` = 已用百分比（高=危险，`colorize`）
- MiniMax `Percent` = 剩余额度（低=危险，`colorize_remaining`），通过 `PROVIDER_REMAINING` 集合标记
